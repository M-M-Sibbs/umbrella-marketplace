// lib/db.js — Turso / libSQL (works on Vercel serverless)
// Local dev with no env vars falls back to a local SQLite file.
...
async function seed(c) {
  ...
  await c.execute({
    sql: `INSERT INTO users (role, first_name, last_name, email, password_hash)
          VALUES ('admin','Site','Admin','admin@umbrellamarketplace.co.zw', ?)`,
    args: [adminHash],
  });
  ...
  const emp = await c.execute({
    sql: `INSERT INTO users (role,first_name,last_name,email,password_hash,company_name,location)
          VALUES ('employer','TechHub','Harare','employer@demo.co.zw',?, 'TechHub Harare','Harare')`,
    args: [demoHash],
  });
  ...
  await c.execute({
    sql: `INSERT INTO users (role,first_name,last_name,email,password_hash,skills,location)
          VALUES ('worker','Tatenda','Moyo','worker@demo.co.zw',?, 'React, Python, Design','Harare')`,
    args: [demoHash],
  });
